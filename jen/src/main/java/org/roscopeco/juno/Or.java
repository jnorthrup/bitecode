...
public class Or extends Constraint
{
   ...
   * or constraint <code>two</code>, or both to be met.
   * 
   * @param one
   * @param two
   */
  public Or(Constraint one, Constraint two) {
    this.one = one;
    this.two = two;    
  }
  ...
  public StringBuilder describe(StringBuilder buffer) {
    return two.describe(one.describe(buffer).append(" or ")).append(", or both");
  }  
}
