...
local_variable_table_length</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
...
;
	...
	LocalVariableTableAttribute()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (LocalVariableTableAttribute LocalVariableTableAttribute : values()) {
		    String hdr = LocalVariableTableAttribute.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    LocalVariableTableAttribute.subIndex(src, register, stack);
		}
	}
