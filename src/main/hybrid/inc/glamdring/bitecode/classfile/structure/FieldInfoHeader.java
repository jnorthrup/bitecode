...
DescriptorIndex</th><td>0</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr><tr><th>
...
,DescriptorIndex,attributes_count	{{
		size=2;
	}}
;
	...
	FieldInfoHeader()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (FieldInfoHeader FieldInfoHeader : values()) {
		    String hdr = FieldInfoHeader.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    FieldInfoHeader.subIndex(src, register, stack);
		}
	}
