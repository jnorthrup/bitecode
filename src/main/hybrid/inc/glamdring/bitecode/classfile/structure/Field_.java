 ...
 * @see inc.glamdring.bitecode.classfile.structure.Field_#ClassIndex
 * @see inc.glamdring.bitecode.classfile.structure.Field_#NameAndTypeIndex
 *</table>
 */
public  enum Field_{
ClassIndex	{{
...
;
	...
	Field_()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (Field_ Field_ : values()) {
		    String hdr = Field_.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    Field_.subIndex(src, register, stack);
		}
	}
