...
line_number</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
...
;
	...
	LineNumberValue()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (LineNumberValue LineNumberValue : values()) {
		    String hdr = LineNumberValue.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    LineNumberValue.subIndex(src, register, stack);
		}
	}
