...
AttributeLength</th><td>0</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
...
,AttributeLength;
	...
	AttributeHeader()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (AttributeHeader AttributeHeader : values()) {
		    String hdr = AttributeHeader.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    AttributeHeader.subIndex(src, register, stack);
		}
	}
