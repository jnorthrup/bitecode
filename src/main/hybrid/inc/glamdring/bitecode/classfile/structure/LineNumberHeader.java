...
AttributeLength</th><td>0</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr><tr><th>

LineNumberCount</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
 * @see inc.glamdring.bitecode.classfile.structure.LineNumberHeader#Utf8Index
 * @see inc.glamdring.bitecode.classfile.structure.LineNumberHeader#AttributeLength
 * @see inc.glamdring.bitecode.classfile.structure.LineNumberHeader#LineNumberCount
 *</table>
 */
...
,AttributeLength,LineNumberCount	{{
		size=2;
	}}
;
	...
	LineNumberHeader()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (LineNumberHeader LineNumberHeader : values()) {
		    String hdr = LineNumberHeader.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    LineNumberHeader.subIndex(src, register, stack);
		}
	}
