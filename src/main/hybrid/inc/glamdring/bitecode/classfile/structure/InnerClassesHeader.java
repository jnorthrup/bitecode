...
classes_count</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
...
;
	...
	InnerClassesHeader()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (InnerClassesHeader InnerClassesHeader : values()) {
		    String hdr = InnerClassesHeader.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    InnerClassesHeader.subIndex(src, register, stack);
		}
	}
