...
length</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
 * @see inc.glamdring.bitecode.classfile.structure.Utf8_#length
 *</table>
 */
public  enum Utf8_{
length	{{
...
;
	...
	Utf8_()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (Utf8_ Utf8_ : values()) {
		    String hdr = Utf8_.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    Utf8_.subIndex(src, register, stack);
		}
	}
