...
;
	...
	SyntheticAttribute()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (SyntheticAttribute SyntheticAttribute : values()) {
		    String hdr = SyntheticAttribute.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    SyntheticAttribute.subIndex(src, register, stack);
		}
	}
