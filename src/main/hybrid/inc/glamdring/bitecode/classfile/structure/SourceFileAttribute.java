...
sourcefileIndex</th><td>2</td><td>0</td><td>{@link java.nio.ByteBuffer}</td></tr> *
...
;
	...
	SourceFileAttribute()	{		 if(  isRecord &&subRecord == null) 		if (subRecord == null) {
		    try {
	...
	static void index(ByteBuffer src, int[] register, IntBuffer stack) {
		for (SourceFileAttribute SourceFileAttribute : values()) {
		    String hdr = SourceFileAttribute.name();
		    System.err.println("hdr:pos " + hdr + ':' + stack.position());
		    SourceFileAttribute.subIndex(src, register, stack);
		}
	}
